...
class VideoProcessor:
    ...
    def process_video(self, start_min=None, start_sec=None, crop_duration=None):
        ...
        if start_min:
            command = f"ffmpeg -i {self.video_path} -c:v libvpx-vp9 -ss 00:{start_min}:{start_sec}00 -t 00:00:0{crop_duration}00 -crf 40 -an -vf scale={new_width}:{new_height} -v quiet -y {output_video_path}"
        else:
            command = f"ffmpeg -i {self.video_path} -c:v libvpx-vp9 -crf 40 -an -vf scale={new_width}:{new_height} -v quiet -y {output_video_path}"
